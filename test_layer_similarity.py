...
def gram_rbf(x, threshold=1.0):
    ...
    sq_median_distance = torch.quantile(sq_distances, q=0.5)
...
def center_gram(gram, unbiased=False):
    ...
    """
    if not torch.allclose(gram, gram.T, atol=1e-4, rtol=1e-4):
...
def cka(gram_x, gram_y, debiased=False):
    ...
    scaled_hsic = gram_x.view(-1) @ gram_y.view(-1)
...
def _debiased_dot_product_similarity_helper(
    ...
    return (
        xty
        - n / (n - 2.0) * sum_squared_rows_x @ (sum_squared_rows_y)
...
def feature_space_linear_cka(features_x, features_y, debiased=False):
    ...
    dot_product_similarity = torch.linalg.norm(features_x.T @ (features_y)) ** 2
    normalization_x = torch.linalg.norm(features_x.T @ (features_x))
    normalization_y = torch.linalg.norm(features_y.T @ (features_y))
    ...
    if debiased:
        ...
        dot_product_similarity = _debiased_dot_product_similarity_helper(
            dot_product_similarity, sum_squared_rows_x, sum_squared_rows_y, squared_norm_x, squared_norm_y, n
        )
        normalization_x = torch.sqrt(
            _debiased_dot_product_similarity_helper(
                normalization_x**2, sum_squared_rows_x, sum_squared_rows_x, squared_norm_x, squared_norm_x, n
            )
        )
        normalization_y = torch.sqrt(
            _debiased_dot_product_similarity_helper(
                normalization_y**2, sum_squared_rows_y, sum_squared_rows_y, squared_norm_y, squared_norm_y, n
            )
        )
...
def cca(features_x, features_y):
    ...
    """
    qx, _ = torch.qr(features_x)  # Or use SVD with full_matrices=False.
    qy, _ = torch.qr(features_y)
    return torch.linalg.norm(qx.T @ (qy)) ** 2 / min(features_x.shape[1], features_y.shape[1])
